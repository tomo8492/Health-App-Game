...
final class BuildingNode: SKSpriteNode {
    ...
    // アイソメトリック奥行き感を出すドロップシャドウ
    private let dropShadow = SKShapeNode(ellipseOf: CGSize(width: 52, height: 14))
    ...
    private func setupDropShadow() {
        // 建物底部（y = -40 = size.height/2 * -1）の少し上にアイソメトリック影を配置
        // isometric: 右下方向にオフセット
        dropShadow.fillColor   = UIColor.black.withAlphaComponent(0.22)
        dropShadow.strokeColor = .clear
        dropShadow.position    = CGPoint(x: 5, y: -36)   // やや右下（アイソ的奥行き感）
